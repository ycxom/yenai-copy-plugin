...
export default new class {
  ...
  async post(url, options = {}) {
    ...
    if (options.data) {
      logger.debug("[yenai-copy-plugin]POST request params data: ", options.data)
  ...
  _prepareRequest(options) {
    ...
    logger.debug(`[yenai-copy-plugin] ${options.method.toUpperCase()}请求：${decodeURI(options.url)}`)
    options.headers = {
